...
async function callElf2Hex(env) {
  return new Promise((resolve) => {
    require(['../elf2hex/elf2hex'], (Module) => {
      resolve(Module(env));
    });
  })
}
...
async function elf2hex(code) {
  console.log(`elf2hex:\n${code}`);
  return new Promise((resolve, reject) => {
    const env = {}
    env.preRun = [(m) => m.FS.writeFile("file.elf", code)];
    let out = "";
    env.print = (data) => {
        out += data + "\n";
    }
    ...
    env.postRun = [
      async (m) => {
        console.log(`elf2hexOut:\n${out}`);
        if (!error) {
          const data = await m.FS.readFile("file.hex")
          resolve(data)
        } else {
          reject(out);
        }
    ...
    env.arguments = ["./file.elf", "0", "16G","./file.hex"]
    callElf2Hex(env)
  })
}
...
async function doAssemble(code, ldscript) {
  ...
  // try {
    const hexout = await elf2hex(elf)
    var string = new TextDecoder().decode(hexout);
    console.log(string)
  // } catch {
  //   console.log("error")
  // }
  ...
  // FS.writeFile("file.elf", elf)
  // callMain(["file.elf", "0", "16G","file.hex"])
  // hexout = FS.readFile("file.hex")
  // var string = new TextDecoder().decode(hexout);
  // console.log(string)
